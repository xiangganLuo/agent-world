package com.aworld.infra.service.codegen.inner;
...
import com.aworld.framework.apilog.core.annotation.ApiAccessLog;
import com.aworld.framework.apilog.core.enums.OperateTypeEnum;
...
import com.aworld.framework.common.pojo.PageParam;
import com.aworld.framework.common.pojo.PageResult;
import com.aworld.framework.common.util.collection.CollectionUtils;
import com.aworld.framework.common.util.date.DateUtils;
import com.aworld.framework.common.util.date.LocalDateTimeUtils;
import com.aworld.framework.common.util.object.BeanUtils;
import com.aworld.framework.common.util.object.ObjectUtils;
import com.aworld.framework.common.util.string.StrUtils;
import com.aworld.framework.excel.core.annotations.DictFormat;
import com.aworld.framework.excel.core.convert.DictConvert;
...
import com.aworld.infra.dal.dataobject.codegen.CodegenColumnDO;
import com.aworld.infra.dal.dataobject.codegen.CodegenTableDO;
import com.aworld.infra.enums.codegen.CodegenFrontTypeEnum;
import com.aworld.infra.enums.codegen.CodegenSceneEnum;
import com.aworld.infra.enums.codegen.CodegenTemplateTypeEnum;
import com.aworld.infra.enums.codegen.CodegenVOTypeEnum;
import com.aworld.infra.framework.codegen.config.CodegenProperties;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 aw-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 aw-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.core.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/aw-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/aw-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "aw-${table.moduleName}/" + // 顶级模块
                "aw-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "aw-${table.moduleName}/" + // 顶级模块
                "aw-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "aw-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "aw-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "aw-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
