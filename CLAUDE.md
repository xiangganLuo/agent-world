...
**重要原则**：当代码中存在 TODO 标记时，AI Agent **必须在每次对话回复中明确告知用户**哪些代码需要手动处理，而不是让用户在代码审查时主动发现。

### TODO 告知格式

在回复中必须包含独立的「📋 待处理事项清单」章节，按优先级分类：
...
1. **JSON 序列化/反序列化**：手动实现的简化 JSON 处理需替换为 Jackson/Fastjson
2. **Redis 访问层**：Service 层直接使用 `StringRedisTemplate` 需移至 DAL 层
3. **魔法值替换**：硬编码字符串/数字需提取为常量或枚举
4. **缓存注解规范化**：手动 Redis 操作建议改为 Spring Cache 注解
5. **外部依赖配置**：API Key、第三方服务地址等需配置文件化

### 检查清单

每次代码生成后，AI Agent 必须自查：
- [ ] 是否扫描了所有生成的文件中的 TODO 标记？
- [ ] 是否在回复中明确列出了所有待处理事项？
- [ ] 是否提供了具体的修复方案和代码示例？
- [ ] 是否按优先级分类（高/中/低）？
